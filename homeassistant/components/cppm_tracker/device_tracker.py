...
async def async_setup_scanner(
    ...
    """Set up the legacy Aruba ClearPass device tracker."""
    import_data = {
...
async def async_setup_entry(
    ...
    """Set up the Aruba ClearPass device tracker from a config entry."""
    ...
    def _async_add_new_devices() -> None:
        ...
        new_entities: list[CPPMScannerEntity] = []
        for mac in coordinator.data:
            if mac not in tracked:
                tracked.add(mac)
                new_entities.append(CPPMScannerEntity(coordinator, mac))
        if new_entities:
...
class CPPMScannerEntity(CoordinatorEntity[CPPMDataUpdateCoordinator], ScannerEntity):
    """Representation of a device tracked by Aruba ClearPass."""
    ...
    def is_connected(self) -> bool:
        """Return true if the device is online in ClearPass."""
