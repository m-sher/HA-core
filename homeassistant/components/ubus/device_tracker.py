"""Device tracker for OpenWrt (ubus) routers."""
...
async def async_setup_scanner(
    ...
    """Set up the legacy OpenWrt (ubus) device tracker."""
    import_data = {
        CONF_HOST: config[CONF_HOST],
        CONF_USERNAME: config[CONF_USERNAME],
        CONF_PASSWORD: config[CONF_PASSWORD],
        CONF_DHCP_SOFTWARE: config.get(CONF_DHCP_SOFTWARE, DEFAULT_DHCP_SOFTWARE),
    }
...
async def async_setup_entry(
    ...
    """Set up the OpenWrt (ubus) device tracker from a config entry."""
    ...
    def _async_add_new_devices() -> None:
        ...
        new_entities: list[UbusScannerEntity] = []
        for mac in coordinator.data:
            if mac not in tracked:
                tracked.add(mac)
                new_entities.append(UbusScannerEntity(coordinator, mac))
        if new_entities:
...
class UbusScannerEntity(CoordinatorEntity[UbusDataUpdateCoordinator], ScannerEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        if device := self._device:
            return {"host": device.get("host", self.coordinator.host)}
