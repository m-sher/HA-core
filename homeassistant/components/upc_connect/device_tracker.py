"""Support for UPC Connect Box device tracking using a coordinator."""
...
from .coordinator import UpcConnectConfigEntry, UpcConnectDataUpdateCoordinator
...
async def async_setup_scanner(
    ...
    """Set up the legacy UPC Connect Box device tracker."""
    import_data = {
    ...
    ir.async_create_issue(
        ...
        translation_placeholders={
            "domain": DOMAIN,
            "integration_title": "UPC Connect Box",
        },
    )
    return True
...
async def async_setup_entry(
    ...
    """Set up the UPC Connect Box device tracker from a config entry."""
    ...
    def _async_add_new_devices() -> None:
        ...
        new_entities: list[UpcConnectScannerEntity] = []
        for mac in coordinator.data:
            if mac not in tracked:
                tracked.add(mac)
                new_entities.append(UpcConnectScannerEntity(coordinator, mac))
        if new_entities:
...
class UpcConnectScannerEntity(
    CoordinatorEntity[UpcConnectDataUpdateCoordinator], ScannerEntity
):
    """Representation of a device connected to the UPC Connect Box."""

    def __init__(self, coordinator: UpcConnectDataUpdateCoordinator, mac: str) -> None:
        """Initialize the tracked device."""
        super().__init__(coordinator)
        self._mac = mac
        device = coordinator.data.get(mac)
        hostname = getattr(device, "hostname", None) if device else None
