"""Common fixtures for the Aruba integration tests."""
...
@pytest.fixture
def mock_get_aruba_data() -> Generator[MagicMock]:
    """Mock Aruba data fetching."""
    mock_get_data = MagicMock(return_value=MOCK_DEVICES)
    with (
        patch(
            "homeassistant.components.aruba.coordinator.get_aruba_data",
            new=mock_get_data,
        ),
        patch(
            "homeassistant.components.aruba.config_flow.get_aruba_data",
            new=mock_get_data,
        ),
    ):
        yield mock_get_data
