"""Tests for the FleetGO device tracker."""
...
from .conftest import MOCK_CONFIG, MOCK_USERNAME
...
async def test_legacy_platform_imports_config_entry(
    hass: HomeAssistant, mock_fleetgo_api: MagicMock
) -> None:
...
async def test_legacy_platform_creates_issue_on_cannot_connect(
    ...
    assert issue.translation_placeholders == {"username": MOCK_USERNAME}
...
async def test_setup_entry_retries_when_unavailable(
    ...
    """Test the config entry retries when FleetGO is unavailable."""
    with patch(
