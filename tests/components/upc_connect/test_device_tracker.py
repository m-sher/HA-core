"""Tests for the UPC Connect Box device tracker."""
...
from homeassistant.components.upc_connect.device_tracker import async_setup_scanner
...
async def test_legacy_platform_imports_config_entry(
    hass: HomeAssistant, mock_connect_box: MagicMock
) -> None:
...
async def test_legacy_platform_creates_issue_on_cannot_connect(
    hass: HomeAssistant, mock_connect_box: MagicMock, issue_registry: IssueRegistry
) -> None:
    """Test an issue is raised when the legacy YAML import cannot connect."""
    mock_connect_box.return_value.async_initialize_token.side_effect = (
        ConnectBoxLoginError("fail")
    )
...
async def test_setup_entry_retries_when_router_unavailable(
    ...
    """Test the config entry retries when the router is unavailable."""
    mock_connect_box.return_value.async_get_devices.side_effect = ConnectBoxError(
        "fail"
    )
    mock_config_entry.add_to_hass(hass)
...
async def test_legacy_import_clears_stale_cannot_connect_issue(
    ...
    mock_connect_box.return_value.async_initialize_token.side_effect = None
