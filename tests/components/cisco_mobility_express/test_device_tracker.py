"""Tests for the Cisco Mobility Express device tracker."""

from unittest.mock import AsyncMock, MagicMock, patch

from homeassistant.components.cisco_mobility_express.const import DOMAIN
from homeassistant.components.cisco_mobility_express.device_tracker import (
    async_setup_scanner,
)
...
async def test_legacy_platform_imports_config_entry(
    hass: HomeAssistant, mock_cisco_me: MagicMock
) -> None:
...
async def test_setup_entry_retries_when_controller_unavailable(
    hass: HomeAssistant,
    mock_config_entry: MockConfigEntry,
    mock_cisco_me: MagicMock,
) -> None:
    """Test the config entry retries when the controller fails."""
    with patch(
