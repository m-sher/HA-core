...
from homeassistant.components.quantum_gateway.device_tracker import async_setup_scanner
...
async def test_entities_created(
    ...
    mock_get_quantum_gateway_data: MagicMock,
    ...
    assert any(eid.startswith("device_tracker.desktop") for eid in entity_ids)
    assert any(
        eid.startswith("device_tracker.ff_ff_ff_ff_ff_ff") for eid in entity_ids
    )


async def test_legacy_platform_imports_config_entry(
    hass: HomeAssistant, mock_get_quantum_gateway_data: MagicMock
) -> None:
...
async def test_legacy_platform_creates_issue_on_cannot_connect(
    ...
    mock_get_quantum_gateway_data.return_value = None
...
async def test_legacy_import_clears_stale_cannot_connect_issue(
    ...
    mock_get_quantum_gateway_data.return_value = MOCK_DEVICES
